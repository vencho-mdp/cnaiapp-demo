...
class class_controller {
  async get_classes (req, res) {
    try {
      ...
      // but also uses dashboard (is logged in)
      if (req.headers.authorization?.split?.(' ')[1] && !JSON.parse(req.query?.get_all || 'false')) {
        const { id } = jwt.verify(req.headers.authorization.split(' ')[1], process.env.ACCESS_TOKEN_SECRET)
