...
const { get_news, add_news, update_news, delete_news } = require('../dao/news')
...
class news_service {
  async get_news (url) {
    try {
      ...
      return news.map(({ image_extension, ...el }) => ({
        ...el,
        image:
          image_extension && url + '/api/images/' + el.id + image_extension,
  ...
  async add_news (new_record, file) {
    try {
      if (file?.filename) {
        const [{ id }] = await add_news({
          ...new_record,
          image_extension: path.extname(file.filename)
        })
    ...
    try {
      if (created_file?.filename) {
        ...
        return await update_news({
          ...updates,
          image_extension: path.extname(created_file.filename)
        })
      }
      ...
      if (!image) {
        ...
        if (original_image_extension) {
          await promises.unlink(
            path.join(
              __dirname,
              '..',
              '/images',
              updates.id + original_image_extension
            )
          )
        }
